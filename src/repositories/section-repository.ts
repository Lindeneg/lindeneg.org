...
class SectionRepository {
    ...
    getById(id: string): AsyncResult<MaybeNull<SectionWithPage>, DbError> {
        return this.db.run(
    ...
    create(data: RawModel<PageSection>): AsyncResult<PageSection, DbError> {
        return this.db.run("section-repo.create", this.db.p.pageSection.create({data}));
    }

    update(id: string, data: RawModelUpdate<PageSection>): AsyncResult<PageSection, DbError> {
        return this.db.run("section-repo.update", this.db.p.pageSection.update({where: {id}, data}));
    }

    delete(id: string): AsyncResult<PageSection, DbError> {
