...
class UserService {
    ...
    async login(
        input: LoginInput
    ): Promise<Result<{user: User; accessToken: string}, UserError>> {
        ...
        return success({user, accessToken: tokenResult.data});
    }
    ...
    async uploadPhoto(userId: string, image: string): Promise<EmptyResult<UserError>> {
