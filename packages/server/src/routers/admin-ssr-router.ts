...
export function makeAdminSsrRouter(deps: AdminSsrRouterDeps): Router {
    ...
    router.post("/pages/new", async (req, res) => {
        ...
        const result = await deps.pageService.createPage({
            name: data.name,
            slug,
            title: data.title,
    ...
    router.post("/pages/:id/edit", async (req, res) => {
        ...
        const updated = await deps.pageService.updatePage(req.params.id, {
            name: data.name,
            slug,
            title: data.title,
            description: data.description,
    ...
    router.post("/pages/:id/delete", async (req, res) => {
        const result = await deps.pageService.deletePage(req.params.id);
        if (!result.ok) return send(res, "Failed to delete page", 500);
        deps.templateService.clearCache();
        res.redirect(302, "/admin/pages");
    });

    router.post("/pages/:id/toggle-publish", async (req, res) => {
        const existing = await deps.pageRepo.getById(req.params.id);
        if (!existing.ok || !existing.data) return send(res, "Page not found", 404);
        const result = await deps.pageService.updatePage(req.params.id, {
    ...
    router.post("/pages/:pageId/sections/new", async (req, res) => {
        ...
        const result = await deps.pageService.createSection({
    ...
    router.post("/sections/:id/edit", async (req, res) => {
        ...
        const result = await deps.pageService.updateSection(req.params.id, parsed.data);
        if (!result.ok) {
            return send(
                res,
                SectionFormView({
    ...
    router.post("/sections/:id/delete", async (req, res) => {
        const found = await findSectionWithPage(req.params.id);
        const result = await deps.pageService.deleteSection(req.params.id);
        if (!result.ok) return send(res, "Failed", 500);
        deps.templateService.clearCache();
        res.redirect(302, found ? `/admin/pages/${found.page.id}/edit` : "/admin/pages");
    });

    router.post("/sections/:id/toggle-publish", async (req, res) => {
        const found = await findSectionWithPage(req.params.id);
        if (!found) return send(res, "Section not found", 404);
        const result = await deps.pageService.updateSection(req.params.id, {
            published: !found.section.published,
        });
        if (!result.ok) return send(res, "Failed", 500);
        deps.templateService.clearCache();
        res.redirect(302, `/admin/pages/${found.page.id}/edit`);
    ...
    router.post("/navigation", async (req, res) => {
        ...
        const result = await deps.navigationService.updateNavigation(navResult.data.id, parsed.data);
        if (!result.ok) {
            return send(
                res,
                NavView({
    ...
    router.post("/nav-items/new", async (req, res) => {
        ...
        const result = await deps.navigationService.createNavItem(parsed.data);
        if (!result.ok) {
            return send(
                res,
                NavItemFormView({
    ...
    router.post("/nav-items/:id/edit", async (req, res) => {
        ...
        const result = await deps.navigationService.updateNavItem(req.params.id, parsed.data);
        if (!result.ok) {
            return send(
                res,
                NavItemFormView({
    ...
    router.post("/nav-items/:id/delete", async (req, res) => {
        const result = await deps.navigationService.deleteNavItem(req.params.id);
        if (!result.ok) return send(res, "Failed", 500);
        deps.templateService.clearCache();
        res.redirect(302, "/admin/navigation");
    });
    ...
    router.post("/blog/:id/toggle-publish", async (req, res) => {
        const existing = await deps.postRepo.getById(req.params.id);
        if (!existing.ok || !existing.data) return send(res, "Post not found", 404);
        const result = await deps.postService.updatePost(req.params.id, {
            published: !existing.data.published,
        });
        if (!result.ok) return send(res, "Failed", 500);
        deps.templateService.clearCache();
    ...
    router.post("/messages/:id/toggle-read", async (req, res) => {
        ...
        const result = await deps.contactService.updateMessage(req.params.id, {read: !found.read});
        if (!result.ok) return send(res, "Failed", 500);
        res.redirect(302, "/admin/messages");
    });

    router.post("/messages/:id/delete", async (req, res) => {
        const result = await deps.contactService.deleteMessage(req.params.id);
        if (!result.ok) return send(res, "Failed", 500);
        res.redirect(302, "/admin/messages");
    });
