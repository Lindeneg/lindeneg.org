...
(async () => {
    ...
    const getPayloadFromRow = (
        ...
        childNodes.forEach((_cell) => {
            const cell = _cell as HTMLTableCellElement;
            if (cell.id === 'table-actions') {
                return;
            }
            payload[cell.dataset.columnName!] = transform(cell.dataset.columnName!, cell.innerText);
            didAdd = true;
        });
    ...
    const handleRowRequest = async (type: string, row: HTMLTableRowElement, navId: string) => {
        ...
        const [payload, didAdd] = getPayloadFromRow(row, { navigationId: navId }, (col, val) => {
